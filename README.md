...
- When installing LibreWolf, the [official installer](https://librewolf.net/installation/windows/) will show an option to install WinUpdater. On first run, it will copy itself to `%AppData%\LibreWolf\WinUpdater` to be able to update itself without administrator privileges.  
  Alternatively, you can download and extract the latest [`LibreWolf-WinUpdater_x.x.x.zip`](https://codeberg.org/librewolf/winupdater/releases) to a like `%AppData%\LibreWolf\WinUpdater`. Run `LibreWolf-WinUpdater.exe` to check for an update. If one is available, it will be downloaded immediately.

## Scheduled updates
- When installing LibreWolf, selecting the option `Schedule Automatic Updates` does automatically what's described in the point below.
...
## Issues
- If you get the error message `Signature verification has failed`, you may have set up a firewall rule that blocks PowerShell (e.g. `%WinDir%\SysWOW64\WindowsPowerShell\v1.0\powershell.exe`). PowerShell sporadically needs internet access for the signature verification process. If you really need to disable signature checks (which is less secure), you can set `NoSigChecks` to `1` in the .ini file under `[Settings]`.
  ```ini
  [Settings]
  NoSigChecks=1
  ```
...
- If you're having issues with the updater on __Windows 7__  (not officially supported by LibreWolf anymore), please have a look at [these instructions](https://codeberg.org/librewolf/winupdater/issues/15).
